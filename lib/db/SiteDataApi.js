...
export async function fetchGlobalAllData({
  ...
  const cachedData = await getOrSetDataWithCache(cacheKey, async () => {
    ...
    const sanitizedData = handleDataBeforeReturn(deepClone(data))
    if (sanitizedData.__NOTION_FETCH_FAILED) {
      // A rejected ISR refresh preserves the last generated page. Caching this
      // fallback would instead replace a healthy site with an error page.
      throw new Error(`Notion returned no usable data for ${pageId}`)
    }
