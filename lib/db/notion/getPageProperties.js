...
export default async function getPageProperties(
  ...
  properties.publishDate = getPublishTimestamp(
    properties?.date,
    value.created_time
  )
...
/**
 * 将 Notion 的发布日期转换为时间戳。
 *
 * notion-utils 会把日期时间拆成 start_date 与 start_time。旧逻辑只读取
 * start_date，导致同一天发布的文章拥有完全相同的排序键，首页只能退回
 * Notion 的手工行顺序。
 */
export function getPublishTimestamp(date = {}, fallback) {
  const {
  ...
  const timestamp = startDate
    ? startTime
      ? getZonedTimestamp(startDate, startTime, timeZone)
...
/**
 * 将 Notion 的本地日期时间转换为 UTC 时间戳。
 *
 * Notion 既可能返回 IANA 时区（如 Asia/Singapore），也可能返回 UTC+8
 * 这样的固定偏移。后者不能直接交给 Intl.DateTimeFormat，否则会被旧的
 * 定时发布代码错误地按 UTC 处理。
 */
function getZonedTimestamp(date, time, timeZone) {
  const dateParts = date.split('-').map(Number)
  const timeParts = time.split(':').map(Number)
  const [year, month, day] = dateParts
  const [hour = 0, minute = 0, second = 0] = timeParts
  const utcGuess = Date.UTC(year, month - 1, day, hour, minute, second)
  const fixedOffset = getFixedUtcOffsetMinutes(timeZone)

  if (fixedOffset !== null) {
    return utcGuess - fixedOffset * 60 * 1000
  }

  try {
    let offset = getIanaTimeZoneOffset(utcGuess, timeZone)
    let timestamp = utcGuess - offset
    const correctedOffset = getIanaTimeZoneOffset(timestamp, timeZone)
    ...
    console.warn(
      `Warning: Unsupported time zone "${timeZone}". Using Asia/Shanghai.`
    )
    return utcGuess - 8 * 60 * 60 * 1000
  }
}

function getFixedUtcOffsetMinutes(timeZone) {
  const normalized = String(timeZone || '').trim()
  if (/^(UTC|GMT|Etc\/GMT(?:\+0)?)$/i.test(normalized)) return 0
...
function getIanaTimeZoneOffset(timestamp, timeZone) {
